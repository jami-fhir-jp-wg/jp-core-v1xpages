# 利用上の注意事項 - HL7 FHIR JP Core ImplementationGuide v1.3.0-dev
...
日本の医療関連法令により、医療文書には法定保存期間が定められている。実装ベンダは、システムの保存設計においてこれらの法定保存期間を遵守する必要がある (**SHALL**)。 保存期間の詳細については [セキュリティ](security.md) ページの「医療文書の保存期間」を参照すること。

...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:FamilyMemberHistory">
      ...
      <sch:assert test="not (*[starts-with(local-name(.), 'age')] and *[starts-with(local-name(.), 'birth')])">年齢[x]または生まれ[x]を持つことができますが、両方ではありません / Can have age[x] or born[x], but not both (inherited)</sch:assert>
      <sch:assert test="exists(*[starts-with(local-name(.), 'age')]) or not(exists(f:estimatedAge))">年齢[x]が存在する場合にのみ推定される可能性があります / Can only have estimatedAge if age[x] is present (inherited)</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
